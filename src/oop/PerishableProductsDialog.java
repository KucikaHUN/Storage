 ...
 * @author kucik
 */
class PerishableProductsDialog extends javax.swing.JDialog {

    private ModelsManager model;
    private final DataManager<Perishable> manager;
    private List<Perishable> products;
    private JTableHeader header;
    private final Map<Integer, Function<List<Perishable>, List<Perishable>>> sorter;

    /**
     * Creates new form PerishableProductsDialog
     */
    public PerishableProductsDialog(DataManager<Perishable> manager) {
        initComponents();
    ...
    private void initComponents() {
        ...
        productsTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Article Number", "Name", "Brand", "Family", "Netto Price", "Brutto Price", "Tax", "Quantity", "Critical Quantity", "Amount Units", "Expiration Date", "Production Date", "Critical Quantity", "Expiration Days"
            }
        ) {
    ...
    private void productsBelowCriticalQuantityBtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_productsBelowCriticalQuantityBtActionPerformed
        try {
            products = (List<Perishable>) manager.getProductsBelowCriticalQuantity(products);
            fillTable(products);
    ...
    private void addProductBtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addProductBtActionPerformed
        PerishableProductEditorDialog editorDialog
                = new PerishableProductEditorDialog(null, manager, products);
    ...
    private void changeProductBtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_changeProductBtActionPerformed
        ...
        if (selectedRow >= 0) {
            Perishable perishable = products.get(selectedRow);
            PerishableProductEditorDialog editorDialog
                    = new PerishableProductEditorDialog(perishable, manager, products);
            editorDialog.setDefaultCloseOperation(DISPOSE_ON_CLOSE);
            editorDialog.setModal(true);
            editorDialog.addWindowListener(new WindowAdapter() {
                @Override
    ...
    private void deleteProductBtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteProductBtActionPerformed
        ...
        if (selectedRow >= 0) {
            Perishable perishable = products.get(selectedRow);
            int dialogResult = JOptionPane.showConfirmDialog(null, perishable.getId() + " Want to delete?");
            if (dialogResult == JOptionPane.YES_OPTION) {
                manager.deleteEntity(perishable);
                products.remove(perishable);
            }
            fillTable(products);
        } else {
            JOptionPane.showMessageDialog(null, "Product was not chosen");
    ...
    private void increaseQuantityProductBtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_increaseQuantityProductBtActionPerformed
        ...
        if (selectedRow >= 0) {
            try {
                Perishable perishable = products.get(selectedRow);
                String quantity = JOptionPane.showInputDialog(null, perishable.getId(), "Increase Quantity: ", JOptionPane.QUESTION_MESSAGE);
                String regex = "[0-9]+";
                int value;
                if (quantity != null && quantity.matches(regex)) {
                    value = Integer.parseInt(quantity);
                    manager.increaseProductQuantity(perishable, value);
                    int amount = manager.getEntityQuantityIfHaveWithId(perishable);
                    perishable.setQuantity(amount);
                    fillTable(products);
                }
            } catch (ErrorException e) {
                JOptionPane.showMessageDialog(null, e.getMessage());
    ...
    private void decreaseQuantityProductBtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_decreaseQuantityProductBtActionPerformed
        ...
        if (selectedRow >= 0) {
            try {
                Perishable perishable = products.get(selectedRow);
                String quantity = JOptionPane.showInputDialog(null, perishable.getId(), "Decrease Quantity: ", JOptionPane.QUESTION_MESSAGE);
                String regex = "[0-9]+";
                int value;
                if (quantity != null && quantity.matches(regex)) {
                    value = Integer.parseInt(quantity);
                    manager.decreaseProductQuantity(perishable, value);
                    int amount = manager.getEntityQuantityIfHaveWithId(perishable);
                    perishable.setQuantity(amount);
                    fillTable(products);
                }
    ...
    private void searchProductBtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchProductBtActionPerformed
        SearchDialog searchDialog = new SearchDialog(manager, DataManagerType.PERISHABLE_MANAGER, products);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(PerishableProductsDialog.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                PerishableProductsDialog dialog = new PerishableProductsDialog(null);
    ...
    private void fillTable(List<Perishable> products) {
    ...
    private String[] productToStringArray(Perishable product) {
        ...
        String expirationDate = String.valueOf(product.getExpirationDate());
        String productionDate = String.valueOf(product.getProductionDate());
        String haveLoweCriticalQuantity = String.valueOf(product.haveLowCriticalQuantity());
        String expirationDays = String.valueOf(product.getExpirationDaysUntilNow());
    ...
    private Map<Integer, Function<List<Perishable>, List<Perishable>>> fillSorter() {
        Map<Integer, Function<List<Perishable>, List<Perishable>>> result = new HashMap<>();
        ...
        result.put(10, (list) -> manager.sortByProperty(PropertyType.EXPIRATION_DATE, list));
        result.put(11, (list) -> manager.sortByProperty(PropertyType.PRODUCTION_DATE, list));
